...
import { getAllQuotations, deleteQuotation, duplicateQuotation, createQuotation } from '../lib/quotationRepo'
...
export default function Dashboard() {
  ...
  const { isEditor, isViewer, isAdmin } = useAuth()
  ...
  return (
    <main style={{ paddingBottom: 80 }}>
      ...
      <div style={{ borderBottom: '1px solid var(--border)', padding: '48px 0' }}>
        <div className="page-wrap">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            {/* Action Buttons Restricted to Editors */}
            {!isViewer && (
              ...
              {quotations.map(q => {
                ...
                return (
                  <div key={q.id} style={{
                    ...
                    background: 'var(--bg)', marginBottom: 1, transition: 'background 0.1s',
                  }} className="quot-row">
                    <style>{`.quot-row:hover { background: var(--surface) !important; }`}</style>
                    
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
                        <Link to={isEditor ? `/edit/${q.id}` : `/preview/${q.id}`} style={{ fontSize: 16, fontWeight: 600, color: 'var(--text)' }}>
                          {eventName}
                        </Link>
                      ...
                      <div style={{ display: 'flex', gap: 16, fontSize: 13, color: 'var(--text-2)' }}>
                        <span><span style={{ color: 'var(--text-3)' }}>Client:</span> {clientName}</span>
                        {eventDate && <span><span style={{ color: 'var(--text-3)' }}>Date:</span> {fmtDate(eventDate)}</span>}
                        <span><span style={{ color: 'var(--text-3)' }}>Items:</span> {items.length}</span>
                      </div>
                    </div>

                    <div style={{ textAlign: 'right', flexShrink: 0, minWidth: 160 }}>
                      <div style={{ fontSize: 18, fontWeight: 700, fontFamily: 'var(--font-mono)' }}>{fmtRp(grandTotal)}</div>
      ...
      {/* Confirmation Modal */}
      {confirmState && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: 24 }} onClick={() => confirmState.onCancel()}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 20, padding: 40, maxWidth: 480, width: '100%', textAlign: 'center', boxShadow: '0 32px 100px rgba(0,0,0,0.6)' }} onClick={e => e.stopPropagation()}>
            <div style={{ fontSize: 48, marginBottom: 24 }}>{confirmState.type === 'danger' ? '🗑️' : (confirmState.type === 'warning' ? '⚠️' : 'ℹ️')}</div>
            <h2 style={{ fontSize: 24, fontWeight: 900, marginBottom: 16 }}>{confirmState.title}</h2>
            <p style={{ color: 'var(--text-2)', marginBottom: 40, fontSize: 16 }}>{confirmState.message}</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <button 
                className={`btn ${confirmState.type === 'danger' ? 'btn-danger' : 'btn-primary'} btn-lg`} 
                style={{ width: '100%', height: 56, fontSize: 16, fontWeight: 700, background: confirmState.type === 'danger' ? 'var(--red)' : '' }} 
                onClick={() => confirmState.onConfirm()}
              >
                {confirmState.confirmLabel || 'Confirm'}
              </button>
              <button className="btn btn-ghost btn-lg" style={{ width: '100%', height: 56 }} onClick={() => confirmState.onCancel()}>
